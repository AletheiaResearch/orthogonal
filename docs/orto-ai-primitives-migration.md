...
| session page message blocks (`user_message` / `token`) | `Message` (+ `MessageContent` / `MessageAvatar`), `Actions`, `CodeBlock`          | Map the event's role to `from="user" \| "assistant" \| "system"`; render text through `Response`; wire copy/retry to `Action`s.                                                 |
