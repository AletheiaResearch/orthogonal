...
| Event                                    | Level | Key Fields                                                       | Description                                                           |
| ---------------------------------------- | ----- | ---------------------------------------------------------------- | --------------------------------------------------------------------- |
| `repo_image.build_triggered`             | info  | `build_id`, `repo_owner`, `repo_name`, `trace_id`, `request_id`  | Repo image build accepted and dispatched to Modal                     |
| `repo_image.build_complete`              | info  | `build_id`, `provider_image_id`, `replaced_image_id`, `trace_id` | Control plane marked repo image ready after Modal callback            |
| `repo_image.build_failed`                | info  | `build_id`, `error_message`, `trace_id`, `request_id`            | Modal build sandbox reported failure via callback                     |
| `repo_image.callback_auth_failed`        | warn  | `trace_id`, `request_id`                                         | Repo image callback rejected (invalid or missing HMAC auth)           |
| `repo_image.callback_auth_misconfigured` | error | `trace_id`, `request_id`                                         | `INTERNAL_CALLBACK_SECRET` unset while handling repo image callback   |
| `repo_image.callback_success`            | info  | `build_id`, `status`, `duration_ms`                              | Sandbox runtime callback to control plane succeeded (sandbox-runtime) |
| `repo_image.callback_failed`             | error | `build_id`, `status`, `error`, `duration_ms`                     | Sandbox runtime callback exhausted retries (sandbox-runtime)          |
