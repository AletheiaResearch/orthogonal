...
export function Combobox<T = string>({
  ...
  return (
    <div className="relative" ref={containerRef} onKeyDown={handleKeyDown}>
      ...
      {open && (
        <div
          className={`absolute ${directionClasses} left-0 ${dropdownWidth} border-border bg-background z-50 border shadow-lg`}
        >
          {searchable && (
            <div className="border-border-muted border-b p-2">
              <input
                ref={searchInputRef}
                type="text"
                value={query}
                ...
                className="border-border bg-input text-foreground placeholder:text-secondary-foreground focus:ring-ring w-full border px-2 py-1.5 text-sm focus:border-transparent focus:outline-none focus:ring-2"
                aria-expanded={open}
          ...
          >
              ...
              displayItems.map((group, groupIdx) => {
                ...
                return (
                  <div key={group.category} role="group" aria-label={group.category}>
                    <div
                      className={`text-secondary-foreground px-3 py-1.5 text-xs font-medium uppercase tracking-wider ${
                        groupIdx > 0 ? "border-border-muted mt-1 border-t" : ""
                      }`}
                    >
                      {group.category}
                    </div>
            ...
            {hiddenCount > 0 && (
              <div className="border-border-muted text-muted-foreground mt-1 border-t px-3 py-2 text-xs">
  ...
  return (
    ...
    >
      <div className="flex min-w-0 flex-col items-start text-left">
        ...
        {option.description && (
          <span className="text-secondary-foreground max-w-full truncate text-xs">
            {option.description}
          </span>
        )}
      </div>
      {isSelected && <CheckIcon className="text-accent h-4 w-4 flex-shrink-0" />}
    </button>
  );
}
